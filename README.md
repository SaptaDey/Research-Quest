...
graph LR
    subgraph "Stage 1: 🎯 Initialization"
        A1[Research Question] --> A2[Root Node Creation]
        A2 --> A3[Context Analysis]
        A3 --> A4[Scope Definition]
    end
    
    subgraph "Stage 2: 🔍 Decomposition"
        B1[Task Breakdown] --> B2[7 Core Dimensions]
        B2 --> B3[Bias Assessment]
        B3 --> B4[Knowledge Gaps]
    end
    
    subgraph "Stage 3: 💡 Hypothesis Planning"
        C1[Competing Theories] --> C2[Falsification Criteria]
        C2 --> C3[Impact Scoring]
        C3 --> C4[Disciplinary Tags]
    end
    
    subgraph "Stage 4: 📈 Evidence Integration"
        D1[Literature Search] --> D2[Evidence Evaluation]
        D2 --> D3[Bayesian Updates]
        D3 --> D4[Confidence Tracking]
    end
```
